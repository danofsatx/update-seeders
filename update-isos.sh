...
# The seeder urls
seeder_1='http://localhost:9091/transmission/rpc --auth=respins:FedoraLiveISOs'
...
for i in $ISO; 
        do wget --quiet $alt_fed$i -P /home/torrents/ &
done
while [ 1 ]; do fg 2> /dev/null; [ $? == 1 ] && break; done

#for i in $ISO;
#        do chown transmission:transmission /srv/torrents/$i 
#done

echo "The ISOs should be downloaded."
...
#for seed in "$seeder_1" "$seeder_2" "$seeder_3" "$seeder_4"; do
        echo "Updating $seed with $release-$updated"
...
#                transmission-remote $seeder_1 -a $alt_fed$tfile
#        done
#done
